...
async def async_setup_entry(
    ...
    async_add_entities(
        [EskomLoadsheddingCalendar(eskom_loadshedding_coordinator)]
    )



class EskomLoadsheddingCalendar( CoordinatorEntity[EskomLoadsheddingDataCoordinator], CalendarEventDevice ):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if self.coordinator.data is not None:
            if len(self.coordinator.data[ATTR_SCHEDULE]) > 0:
                date_time_start = datetime.strptime(self.coordinator.data[ATTR_SCHEDULE][0][0],date_time_format)
                date_time_end = datetime.strptime(self.coordinator.data[ATTR_SCHEDULE][0][1],date_time_format)
                self._event = {
                        "all_day": False,
